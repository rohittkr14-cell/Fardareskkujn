...
async def user_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "scm_amount":
        if not text.isdigit():
            return await update.message.reply_text("❌ Amount words me nahi\nSirf number bhejo")
    ...
    if step == "imp_fake_id":
        if not text.isdigit():
            return await update.message.reply_text("❌ Fake ID galat hai\nSirf number bhejo")
    ...
    if step == "imp_real_id":
        if not text.isdigit():
            return await update.message.reply_text("❌ Real ID galat hai\nSirf number bhejo")
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in STATE:
        return await q.message.reply_text("❌ Report expired")
    ...
    if action == "approve":
        ...
        # ---- CHANNEL TEXT + PFP ----
        if d["type"] == "SCM":
            caption = f"❌ User {d['scm_username']} (Telegram User ID: {d['scm_id']}) flagged"
            kb = [[
                InlineKeyboardButton(
                    "View Profile",
                    url=f"https://t.me/{d['scm_username'].replace('@','')}"
                ),
                InlineKeyboardButton(
                    "View Proof",
                    url=d["proof"]
            ...
            kb = [[
                InlineKeyboardButton(
                    "View Real Profile",
                    url=f"https://t.me/{d['real_username'].replace('@','')}"
                ),
                InlineKeyboardButton(
                    "View Fake Profile",
                    url=f"https://t.me/{d['fake_username'].replace('@','')}"
                )
            ]]
        ...
        await q.message.edit_text("✅ Approved & action taken")
        reset(uid)

    else:
        await q.message.edit_text("❌ Report rejected")
        reset(uid)
